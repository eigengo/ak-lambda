...
lazy val ingestProtocol = project.in(file("ingest-protocol")).settings(Build.Settings.project)
  .dependsOn(common)

lazy val infrastructure = project.in(file("infrastructure")).settings(Build.Settings.project)

// Data ingestion code
lazy val ingest = project.in(file("ingest")).settings(Build.Settings.project)
  .dependsOn(infrastructure, ingestProtocol, common)

lazy val speedProtocol = project.in(file("speed-protocol")).settings(Build.Settings.project)
  .dependsOn(common)

lazy val speedRules = project.in(file("speed-rules")).settings(Build.Settings.project)
